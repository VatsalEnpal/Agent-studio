...
// Cache for display names (first user message), keyed by sessionId
const displayNameCache = new Map<string, string | null>();

/**
 * Extract the first user message from a JSONL file to use as display name.
 * Reads only the first 16KB to keep it fast.
 */
function extractFirstUserMessage(jsonlPath: string): string | null {
  try {
    const fd = require("node:fs").openSync(jsonlPath, "r");
    const buf = Buffer.alloc(16384);
    const bytesRead = require("node:fs").readSync(fd, buf, 0, 16384, 0);
 ...
 * Uses a cache to avoid re-reading JSONL files on every poll.
 */
...
/**
 * Get display name for a session by looking up the Claude session ID
 * from a PTY PID.
 */
export async function getDisplayNameForPtyPid(
  ptyPid: number,
): Promise<string | null> {
  const claudeSessionId = await findSessionIdForPtyPid(ptyPid);
...
function parseJsonlUsage(
  ...
  try {
    ...
    const contextTotal =
      CONTEXT_WINDOW[modelShort] ?? CONTEXT_WINDOW["unknown"]!;
    const contextPercent =
      contextTotal > 0
        ? Math.min(100, Math.round((contextUsed / contextTotal) * 100))
        : 0;
 ...
 */
export async function findSessionIdForPtyPid(
  ptyPid: number,
): Promise<string | null> {
  const sessionFiles = readSessionFiles();
  ...
  // by checking process tree via pgrep
  try {
    ...
    for (const childPid of childPids) {
      ...
      try {
        const { stdout: grandRaw } = await execFileAsync(
          "pgrep",
          ["-P", String(childPid)],
          { encoding: "utf-8", timeout: 2000 },
        );
        ...
        // pgrep returns exit 1 when no children found
      }
    }
  } catch {
    // pgrep returns exit 1 when no children found
