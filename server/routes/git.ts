...
export function gitRoutes(gitWatcher: GitWatcher): Router {
  ...
  router.get("/status", async (_req, res) => {
    try {
      const statuses = await gitWatcher.getStatus();
      res.json(statuses);
    } catch (err) {
  ...
  router.post("/pr", async (req, res) => {
    try {
      ...
      if (!repo || !sourceBranch || !targetBranch || !title) {
        res.status(400).json({
          error:
            "Missing required fields: repo, sourceBranch, targetBranch, title",
        });
        return;
      }
  ...
  router.post("/commit", async (req, res) => {
    try {
      ...
      if (repoInfo?.isProd) {
        res.status(403).json({
          error:
            "BLOCKED: Cannot commit directly to production repo. Changes require explicit approval from Vatsal.",
        });
        return;
      }
      ...
      const { stdout } = await execAsync(
        `git -c user.name="VatsalBhatt" -c user.email="vatsal.bhatt@enpal.de" commit -m ${JSON.stringify(commitMsg)}`,
