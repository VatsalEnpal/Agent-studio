...
export function systemRoutes(
  ...
  const router = Router();
  const PMO_PLIST = `${os.homedir()}/Library/LaunchAgents/com.inpipeline.pmo-scan.plist`;
  ...
  router.post("/servers/custom", (req, res) => {
    try {
      const { name, cwd, command } = req.body as {
        name?: string;
        cwd?: string;
        command?: string;
      };
  ...
  router.get("/pmo/status", async (_req, res) => {
    try {
      let isLoaded = false;
      try {
        const { stdout } = await execAsync("launchctl list 2>/dev/null", {
          timeout: 5000,
        });
        isLoaded = stdout.includes("inpipeline");
      } catch {
        isLoaded = false;
      }
  ...
  router.get("/pmo/status-full", async (_req, res) => {
    try {
      ...
      const lastEntry =
        scanEntries.length > 0 ? scanEntries[scanEntries.length - 1] : null;
  ...
  router.post("/pmo/start", async (_req, res) => {
    try {
      await execAsync(`launchctl load "${PMO_PLIST}" 2>/dev/null || true`, {
        timeout: 5000,
      });
  ...
  router.post("/pmo/stop", async (_req, res) => {
    try {
      await execAsync(`launchctl unload "${PMO_PLIST}" 2>/dev/null || true`, {
        timeout: 5000,
      });
