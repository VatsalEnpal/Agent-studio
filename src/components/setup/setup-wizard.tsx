...
export function SetupWizard({ onComplete }: SetupWizardProps) {
  ...
  const [permissions, setPermissions] = useState<"bypass" | "default" | "plan">(
    "bypass",
  );
  ...
  const handleFinish = useCallback(async () => {
    ...
    try {
      ...
      if (agentSystem.enabled && agentSystem.createNew && firstProject) {
        setScaffolding(true);
      ...
      <div className="space-y-3">
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          <input
            ...
            className="accent-console-accent"
          />
          <span className="text-xs text-console-text">I have one already</span>
        </label>
      ...
      {agentSystem.enabled && agentSystem.createNew && (
        <div className="pl-6">
          <div className="flex items-start gap-2 px-3 py-2 bg-console-accent/5 border border-console-accent/20 rounded text-[10px] text-console-muted">
            ...
            <span>
              We&apos;ll create{" "}
              <code className="text-console-accent">ai-agents/</code> and{" "}
              <code className="text-console-accent">.claude/agents/</code> in
              your first project directory. Choose your agents on the next page.
            </span>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        >
          ...
          <div>
            ...
            <p className="text-[10px] text-console-muted mt-0.5">
              Get pinged when sprints are ready or gates pass. Requires a
              Telegram bot token (you can set this up later).
            </p>
          </div>
        </label>
        ...
        >
          ...
          <div>
            ...
            <p className="text-[10px] text-console-muted mt-0.5">
              Automatically scan for tasks on a schedule. Requires launchd
              (macOS) or cron (Linux).
            </p>
          </div>
        </label>
...
function ValidationRow({
  ...
  return (
    <div className="flex items-center gap-2 text-[10px]">
      ...
      {detail && <span className="text-console-muted">({detail})</span>}
    </div>
  );
}
    ...
    <div className="space-y-5">
      ...
      {scaffoldResult && scaffoldResult.created.length > 0 && (
        <div className="flex items-start gap-2 px-3 py-2 bg-green-500/5 border border-green-500/20 rounded text-[10px] text-green-400">
          ...
          <span>
            Agent system created: {scaffoldResult.created.length} files
            generated.
          </span>
        </div>
      )}
