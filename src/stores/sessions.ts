...
export const useSessionsStore = create<SessionsState>((set, get) => ({
  ...
  zoomIn: (id) => {
    ...
    const next =
      (idx >= 0 && idx < ZOOM_LEVELS.length - 1
        ? ZOOM_LEVELS[idx + 1]
        : undefined) ?? current;
    const newLevels: Record<string, number> = Object.create(null);
    for (const [k, v] of Object.entries(state.zoomLevels)) {
      newLevels[k] = v;
    }
    newLevels[id] = next;
    set({ zoomLevels: newLevels });
  },
  ...
  zoomOut: (id) => {
    ...
    const current = state.zoomLevels[id] ?? DEFAULT_ZOOM;
    const idx = ZOOM_LEVELS.indexOf(current as (typeof ZOOM_LEVELS)[number]);
    const prev = (idx > 0 ? ZOOM_LEVELS[idx - 1] : undefined) ?? current;
    const newLevels: Record<string, number> = Object.create(null);
    for (const [k, v] of Object.entries(state.zoomLevels)) {
      newLevels[k] = v;
    }
