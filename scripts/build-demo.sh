...
echo "Step 1: Trimming 18s starting at ${SKIP}s, scaling to 720p..."
$FFMPEG -y -ss "$SKIP" -i "$INPUT" \
  -t 18 \
  -vf "fade=in:0:5,fade=out:st=17:d=1,scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2:color=#0a0a0a" \
...
echo "Step 1b: Trimming 18s starting at ${SKIP}s (1080p)..."
$FFMPEG -y -ss "$SKIP" -i "$INPUT" \
  -t 18 \
  -vf "fade=in:0:5,fade=out:st=17:d=1,scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=#0a0a0a" \
...
$FFMPEG -y \
  ...
  -vf "drawtext=text='Agent Studio':fontcolor=0xf59e0b:fontsize=56:x=(w-text_w)/2:y=(h/2)-40:font='SF Mono',drawtext=text='AI agents, working in parallel':fontcolor=0x71717a:fontsize=22:x=(w-text_w)/2:y=(h/2)+30:font='SF Mono',fade=in:0:10" \
  -c:v libx264 -crf 22 -preset medium -r 30 \
  -pix_fmt yuv420p \
  -t 3 \
  "$ENDCARD" 2>/dev/null
...
$FFMPEG -y \
  ...
  -vf "drawtext=text='Agent Studio':fontcolor=0xf59e0b:fontsize=72:x=(w-text_w)/2:y=(h/2)-50:font='SF Mono',drawtext=text='AI agents, working in parallel':fontcolor=0x71717a:fontsize=28:x=(w-text_w)/2:y=(h/2)+40:font='SF Mono',fade=in:0:10" \
  -c:v libx264 -crf 20 -preset medium -r 30 \
  -pix_fmt yuv420p \
  -t 3 \
...
$FFMPEG -y \
  -i "$TRIMMED" -i "$ENDCARD" \
  -filter_complex "[0:v][1:v]xfade=transition=fade:duration=0.3:offset=17.7[v]" \
  -map "[v]" \
  -c:v libx264 -crf 22 -preset medium -r 30 \
  -pix_fmt yuv420p \
  -movflags +faststart \
